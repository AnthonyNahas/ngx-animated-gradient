...
const config = {
  libraryName: 'ngx-animated-gradient',
  unscopedLibraryName: 'ngx-animated-gradient',
  allSrc: 'src/**/*',
  allTs: 'src/**/!(*.spec).ts',
...
gulp.task('build:demo', () => {
  return execDemoCmd(`build --preserve-symlinks --prod --base-href /ngx-animated-gradient/ --deploy-url /ngx-animated-gradient/`, {cwd: `${config.demoDir}`});
});
...
gulp.task('build:demo-ssr', () => {
  return execDemoCmd(`build --preserve-symlinks --prod`, {cwd: `${config.demoDir}`})
    .then(() => execDemoCmd(`run ngx-animated-gradient-demo:server`, {cwd: `${config.demoDir}`}))
